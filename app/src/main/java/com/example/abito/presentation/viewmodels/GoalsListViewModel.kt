...
class GoalsListViewModel @Inject constructor(
    ...
    private fun updateStateWithNewStreak(goalType: GoalType, newStreak: Streak): List<Goal> {
        if (goalType == GoalType.START) {
            return state.goals.map { goal ->
                if (goal.id != newStreak.goalId) return@map goal

                return@map goal.copy(activeStreak = newStreak)
            }
        }
        ...
        return state.goals.map { goal ->
            ...
            val modifiedPreviousStreaks = (goal.previousStreaks.orEmpty()) + newStreak

            return@map goal.copy(activeStreak = null, previousStreaks = modifiedPreviousStreaks)
        }
    }
    ...
    fun updateStartStreak(goalId: GoalId, streakId: StreakId) {
        viewModelScope.launch {
            ...
            when (val result = updateStartStreakUseCase(goalId, streakId)) {
                is Resource.Success -> {
                    state = state.copy(
                        goals = updateStateWithNewStreak(GoalType.START, result.data)
                    )
                }
    ...
    fun endStopStreak(goalId: GoalId, streakId: StreakId) {
        viewModelScope.launch {
            ...
            when (val result = endStopStreakUseCase(goalId, streakId)) {
                is Resource.Success -> {
                    state = state.copy(
                        goals = updateStateWithNewStreak(GoalType.STOP, result.data)
                    )
                }
